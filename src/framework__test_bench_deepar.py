...
class TestBenchDeepAR:
    ...
    *******************************************************************************************************************
    ...
    def __to_dataloaders(self, data, max_encoder_length, max_prediction_length, batch):
        # define last idx for train and last idx for validation
        ...
        training = TimeSeriesDataSet(
            ...
            static_categoricals=["device"],
            # as we plan to forecast correlations, it is important to use series characteristics (e.g. a series
            # identifier)
            categorical_encoders={"device": NaNLabelEncoder(add_nan=True).fit(data.device),
                                  "series": NaNLabelEncoder(add_nan=True).fit(data.series)},
            time_varying_unknown_reals=["value"],
            max_encoder_length=max_encoder_length,
            max_prediction_length=max_prediction_length
        )
        validation = TimeSeriesDataSet.from_dataset(training, data[lambda x: x.time_idx <= validation_cutoff],
    ...
    def __look_at_me(self, df, num_long_to_predict=4):
        # choosing randomly time series to predict over
        random.seed("313")
        ...
        edited_long_ds = {}
        # define big series num to distinguish it from the model series
        series_num = 220000
    ...
    def __do_one_test(self, dictionary):
        metric, app, max_prediction_length, plot, batch, max_epochs = dictionary["metric"], dictionary["app"], \
            dictionary["prediction length"], dictionary["plot"], dictionary["batch"], dictionary["max epochs"]
        ...
        train_dataloader, val_dataloader, test_dataloader, training, validation, test = (
            self.__to_dataloaders(dataset,
                                  max_encoder_length,
        ...
        model.learn_from_data_set(train_dataloader, val_dataloader, max_epochs)
